...
# 从头开始写React和vue的Webpack配置

使用Webpack从头搭建一个React项目，包括下面的几个部分

* 基础配置：资源的输入和输出等
* JavaScript处理：babel以及React相关的插件
* TypeScript处理：typescript和ts-lader
* 样式处理：less、CSS在不同环境下的处理
* 静态资源处理：字体、图片的加载
* 优化：开发环境和生产环境的配置调整。
...
使用TypeScript编写入口文件并导入less样式,测试项目的渲染结果

由于项目中使用到了Node环境的 `process.env.NODE_ENV`需要修改启动命令或是使用.env文件配置

> 参考文章：[webpack环境配置之process.env - 掘金 (juejin.cn)](https://juejin.cn/post/6989812435561480200)
